...
impl ApiKeyEncryptionService {
    ...
    pub fn decrypt_api_key(&self, encrypted_api_key: &str) -> Result<String> {
        // Decode from base64
        let combined = BASE64.decode(encrypted_api_key)
    ...
    /// Get or generate the API key encryption key
    fn get_or_generate_api_key_encryption_key(encryption_key_service: &EncryptionKeyService) -> Result<[u8; 32]> {
        ...
        encryption_key_service.store_key(&key_str)
    ...
    pub fn test_encryption(&self) -> Result<()> {
        ...
        if test_api_key != decrypted {
            return Err(anyhow!("Encryption test failed: decrypted value doesn't match original"));
        }
