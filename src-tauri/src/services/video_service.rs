...
use std::io::Read;
use crate::{app_log_info, app_log_error, app_log_debug, app_log_warn};
...
impl VideoService {
    ...
    pub fn is_ffmpeg_available(&self) -> bool {
        ...
        app_log_info!("🔍 Checking FFmpeg availability: {}", if has_both { "✅ available" } else { "❌ not available" });
        has_both
    }
    ...
    pub fn get_video_metadata(&self, video_path: &str) -> Result<VideoMetadata> {
        ...
        let ffprobe = match &self.ffprobe_path {
            Some(path) => path,
            None => return Err(anyhow!("Bundled FFprobe not found, cannot get video metadata")),
        };
        ...
        cmd.args([
            ...
            "-show_entries", "stream=width,height,duration,codec_name,bit_rate,avg_frame_rate", // Get more metadata efficiently
            "-of", "json",                    // JSON output for easy parsing
            video_path,
        ]);
    ...
    fn analyze_video_characteristics(&self, metadata: &VideoMetadata, video_path: &str) -> Result<VideoAnalysis> {
    ...
    fn calculate_smart_sampling_rate(&self, analysis: &VideoAnalysis, requested_fps: f32) -> f32 {
        ...
        let min_fps = if analysis.content_type == "static" { 0.02 } else { 0.1 };
    ...
    fn extract_jpeg_frames(buffer: &[u8]) -> (Vec<Vec<u8>>, Vec<u8>) {
        ...
        while i < buffer.len().saturating_sub(1) {
            if buffer[i] == 0xFF && buffer[i + 1] == 0xD9 {  // End of Image marker
                ...
                while j < buffer.len().saturating_sub(1) {
                    if buffer[j] == 0xFF && buffer[j + 1] == 0xD8 {  // Start of Image marker
                        start = j;
                        i = j + 1;
                        break;
                    }
        ...
        F: FnMut(image::DynamicImage, VideoFrameMetadata) -> Result<()>
    {
        ...
        let video_name = video_file.file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("unknown");
        ...
        app_log_info!("📊 Video metadata: {}x{}, {:.1}s duration, {} codec", 
        ...
        app_log_info!("🎯 Video Analysis: content_type='{}', motion_level={:.2}, smart_fps={}", 
            video_analysis.content_type, video_analysis.motion_level, smart_fps);
        ...
        if let Some(ref progress_cb) = progress_callback {
            progress_cb(VideoProcessingProgress {
                ...
                current_operation: format!("Starting in-memory frame extraction ({} frames expected)...", estimated_frames),
                fps: smart_fps,
        ...
        if let Some(max_res) = max_resolution {
            vf_filters.push(format!("scale='min({},iw)':'min({},ih)':force_original_aspect_ratio=decrease", max_res, max_res));
        }
        ...
        cmd.arg("-i")
            ...
            .args([
                ...
                "-c:v", "mjpeg",
                "-q:v", "3",
                "-f", "image2pipe",  // Pipe output instead of files
                    ...
                    for frame_bytes in frames {
                        ...
                        if enable_scene_detection {
                            ...
                            if let Some(last_hash) = last_frame_hash {
                                let similarity = Self::calculate_hash_similarity(last_hash, current_hash);
                                
                                if similarity > similarity_threshold {
                                    skipped_frames += 1;
                                    frame_skipped = true;
                                    app_log_debug!("🔄 Skipping similar frame {} (similarity: {:.3})", frame_count, similarity);
                                } else {
                                    app_log_debug!("✅ Keeping frame {} (similarity: {:.3})", frame_count, similarity);
                                }
                            }
                        ...
                        if frame_count % 10 == 0 {
                            ...
                            let processing_speed = if elapsed > 0.0 { frame_count as f64 / elapsed } else { 0.0 };
                            ...
                            if let Some(ref progress_cb) = progress_callback {
                                progress_cb(VideoProcessingProgress {
                                    ...
                                    current_operation: format!("Processing frame {} (in-memory)", frame_count),
                                    fps: smart_fps,
                                    video_duration: video_info.duration,
                                    estimated_frames,
                                    processing_speed,
        ...
        if !exit_status.success() {
            app_log_error!("❌ FFmpeg process failed with exit code: {:?}", exit_status.code());
            return Err(anyhow!("FFmpeg process failed"));
        }

        let total_time = start_time.elapsed();
        app_log_info!("✅ In-memory video processing completed: {} frames ({} skipped) in {:.1}s", 
            frame_count, skipped_frames, total_time.as_secs_f64());
        ...
        if let Some(ref progress_cb) = progress_callback {
            progress_cb(VideoProcessingProgress {
                ...
                current_operation: format!("Completed: {} frames processed in-memory", frame_count - skipped_frames),
                fps: smart_fps,
                video_duration: video_info.duration,
