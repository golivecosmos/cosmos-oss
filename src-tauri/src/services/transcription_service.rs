    ...
    pub fn store_transcription(&self, transcription_result: &crate::services::audio_service::TranscriptionResult, file_path: &str) -> Result<String> {
        ...
        db.execute(
            ...
            rusqlite::params![
                ...
                transcription_result.segments.first().and_then(|s| s.confidence)
            ],
        )?;
    ...
    pub fn get_transcription_by_path(&self, file_path: &str) -> Result<Option<serde_json::Value>> {
        ...
        let result = db.query_row(
            ...
            |row| {
                ...
                let segments: serde_json::Value = serde_json::from_str(&segments_json)
...
mod tests {
    ...
    fn test_transcription_storage_and_retrieval() {
        ...
        let db = connection.lock().unwrap();
        schema_service.create_transcriptions_table(&db).expect("Failed to create transcriptions table");
        drop(db);
        ...
        let id = transcription_service.store_transcription(&transcription_result, file_path).expect("Failed to store transcription");
        assert!(!id.is_empty());

        // Retrieve transcription
        let retrieved = transcription_service.get_transcription_by_path(file_path).expect("Failed to get transcription");
        assert!(retrieved.is_some());
        
        let transcription = retrieved.unwrap();
        assert_eq!(transcription["transcription_text"].as_str().unwrap(), "Hello world");
