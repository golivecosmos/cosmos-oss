...
/// Supported text/document file extensions for semantic chunk indexing
pub const SUPPORTED_TEXT_EXTENSIONS: &[&str] = &[
    "txt", "md", "markdown", "json", "csv", "log", "xml", "yml", "yaml", "toml", "ini", "conf",
    "rtf", "pdf",
];
...
pub fn is_supported_text_extension(ext: &str) -> bool {
...
pub fn is_supported_media_extension(ext: &str) -> bool {
    ...
    SUPPORTED_IMAGE_EXTENSIONS.contains(&ext_lower.as_str())
        ...
        || SUPPORTED_TEXT_EXTENSIONS.contains(&ext_lower.as_str())
}
...
pub fn model_registry_base_url() -> String {
    std::env::var("COSMOS_MODEL_BASE_URL").unwrap_or_else(|_| DEFAULT_MODEL_BASE_URL.to_string())
}

/// Optional namespace/organization segment (defaults to `nomic-ai`).
pub fn model_namespace() -> String {
    std::env::var("COSMOS_MODEL_NAMESPACE").unwrap_or_else(|_| DEFAULT_MODEL_NAMESPACE.to_string())
}

/// Text embedding model slug (can include extra path segments like `resolve/main`).
pub fn text_model_slug() -> String {
    std::env::var("COSMOS_TEXT_MODEL_SLUG").unwrap_or_else(|_| DEFAULT_TEXT_MODEL_SLUG.to_string())
}
