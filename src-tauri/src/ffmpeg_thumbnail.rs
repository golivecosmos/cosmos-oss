...
pub async fn generate_ffmpeg_thumbnail(
    file_path: &str,
    timestamp: f64,
    width: u32,
    height: u32,
    ...
    let output = Command::new(&ffmpeg_path)
        ...
        .arg("-vf")
        .arg(&format!(
            "scale={}:{}:flags=lanczos:force_original_aspect_ratio=decrease:eval=frame",
            width, height
        ))
        .arg("-q:v")
        .arg("2")
    ...
    let jpeg_data =
        std::fs::read(&jpeg_path).map_err(|e| format!("Failed to read thumbnail: {}", e))?;
