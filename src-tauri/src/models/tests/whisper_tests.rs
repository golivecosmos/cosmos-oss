use crate::models::whisper::{TranscriptionResult, TranscriptionSegment, WhisperModel};
...
fn test_timestamp_conversion() {
    ...
    for (token, expected_seconds) in test_cases {
        ...
        assert!(
            (calculated_seconds - expected_seconds).abs() < 0.001,
...
fn test_transcription_result_word_segments() {
    ...
    for (i, segment) in result.segments.iter().enumerate() {
        ...
        assert!(
            ...
            "Word '{}' duration should be 0-1 seconds: {:.2}s",
            segment.text,
            duration
        );
    }
...
fn test_language_detection() {
    ...
    for lang in supported_languages {
        ...
        assert!(
            token_format.starts_with("<|") && token_format.ends_with("|>"),
            "Language token should be in Whisper format: {}",
            token_format
        );
        assert!(
            token_format.len() >= 5,
            "Language token should be at least 5 characters: {}",
            token_format
        );
    }
}
...
fn test_transcription_quality_metrics() {
    ...
    assert!(
        high_avg > 0.9,
        "High quality should have >90% confidence: {:.2}",
        high_avg
    );
    assert!(
        low_avg < 0.5,
        "Low quality should have <50% confidence: {:.2}",
        low_avg
    );
    assert!(
        high_avg > low_avg,
        "High quality should exceed low quality confidence"
    );
}
...
fn test_word_boundary_detection() {
    ...
    assert_eq!(
        segments.len(),
        words.len(),
        "Should have one segment per word"
    );
    ...
    for (i, segment) in segments.iter().enumerate() {
        ...
        assert_eq!(
            clean_segment, clean_word,
            "Segment should match word: '{}' vs '{}'",
            segment.text, words[i]
        );
        ...
        let duration = segment.end - segment.start;
        assert!(
            (duration - word_duration).abs() < 0.001,
            "Segment duration should match expected: {:.3}s",
            duration
        );
    }
}
