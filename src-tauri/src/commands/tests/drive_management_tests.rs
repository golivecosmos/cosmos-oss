...
async fn test_drive_metadata_operations() {
    ...
    // Test adding a drive
    let result = sqlite_service.add_drive(&drive_uuid, "Test Drive", "/test/path", true);
...
async fn test_drive_status_updates() {
    ...
    // Add a test drive
    sqlite_service
        .add_drive(&drive_uuid, "Test Drive", "/test", true)
...
async fn test_drive_path_detection() {
    ...
    let system_paths = vec!["/usr/bin/ls", "/System/Library", "", "/nonexistent/path"];
    ...
    for path in system_paths {
        ...
        assert!(
            ...
            "get_drive_for_path should complete for: {}",
            path
        );
    }
}
...
fn test_drive_validation_edge_cases() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            Some("Very Long Physical Location Description".to_string()),
        ), // Long strings
    ];
    ...
    for (name, location) in test_cases {
        ...
        assert!(
            name.is_none() || !name.as_ref().unwrap().contains('\0'),
            "Names should not contain null bytes"
        );
        assert!(
            location.is_none() || !location.as_ref().unwrap().contains('\0'),
