...
pub async fn get_jobs(
    ...
    match result {
        ...
        Err(e) => {
            ...
            if error_str.contains("no such table: jobs")
                || error_str.contains("jobs") && error_str.contains("not found")
            {
                ...
                match state.sqlite_service.recover_from_jobs_table_error() {
                    Ok(_) => {
                        ...
                        match retry_result {
                            Ok(jobs) => {
                                app_log_info!(
                                    "✅ JOBS: Retrieved {} jobs after recovery",
                                    jobs.len()
                                );
                                Ok(jobs)
                            }
                            Err(retry_e) => {
                                app_log_error!(
                                    "❌ JOBS: Failed to get jobs even after recovery: {}",
                                    retry_e
                                );
...
pub async fn manage_job_queue(
    ...
    app_log_info!(
        "🔍 BACKEND: manage_job_queue called with action: '{}', job_id: {:?}",
        action,
        job_id
    );
    match action.as_str() {
            ...
            match state.sqlite_service.get_queue_health_snapshot(600) {
                Ok(mut queue_status) => {
                    if let Some(status_obj) = queue_status.as_object_mut() {
                        status_obj.insert(
                            "paused".to_string(),
                            serde_json::json!(is_queue_processing_paused()),
                        );
                    }
                    Ok(queue_status)
                }
        ...
        "clear" => {
            ...
            match state.sqlite_service.clear_jobs_queue() {
                Ok(deleted_count) => {
                    app_log_info!(
                        "✅ CLEAR QUEUE: Successfully cleared {} jobs",
                        deleted_count
                    );
        ...
        "clear_all" => {
            ...
            match state.sqlite_service.clear_all_jobs() {
                Ok(deleted_count) => {
                    app_log_info!(
                        "✅ CLEAR ALL JOBS: Successfully deleted {} jobs",
                        deleted_count
                    );
    ...
    match action.as_str() {
        "cancel_all_pending" => {
            ...
            match state.sqlite_service.get_jobs_by_status("pending") {
                Ok(pending_jobs) => {
                    ...
                    for job in pending_jobs {
                        ...
                        match state.sqlite_service.cancel_job(job_id_str) {
                            ...
                            Err(e) => {
                                app_log_error!(
                                    "❌ CANCEL FAILURE: Failed to cancel job {}: {}",
                                    job_id_str,
                                    e
                                );
                            }
                ...
                Err(e) => {
                    app_log_error!(
                        "❌ CANCEL ALL PENDING JOBS: Failed to get pending jobs: {}",
                        e
                    );
    ...
    if let Ok(temp_dir) = get_job_temp_directory(job_id, job_path) {
        if temp_dir.exists() {
            match std::fs::remove_dir_all(&temp_dir) {
                ...
                Err(e) => {
                    app_log_error!(
                        "❌ CLEANUP: Failed to remove temp directory {}: {}",
                        temp_dir.display(),
                        e
                    );
                }
...
fn get_job_temp_directory(job_id: &str, job_path: &str) -> Result<std::path::PathBuf, String> {
    ...
    if job_path.ends_with(".mp4") || job_path.ends_with(".mov") || job_path.ends_with(".avi") {
        ...
        let temp_dir = app_data_dir
            .join("temp")
            .join(format!("video_processing_{}", std::process::id()));
        Ok(temp_dir)
    } else {
