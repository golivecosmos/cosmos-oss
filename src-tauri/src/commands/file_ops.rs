...
use crate::{app_log_info, app_log_error, app_log_warn, app_log_debug};
...
#[tauri::command]
pub fn list_directory_contents(path: String, state: State<'_, AppState>) -> Result<Vec<FileItem>, String> {
    state.file_service.list_directory(&path)
...
pub fn get_file_metadata(path: String, state: State<'_, AppState>) -> Result<FileMetadata, String> {
    state.file_service.get_file_metadata(&path)
...
pub fn read_file_content(path: String, state: State<'_, AppState>) -> Result<String, String> {
    state.file_service.read_file_content(&path)
...
pub async fn read_file_as_base64(path: String) -> Result<String, String> {
    ...
    match fs::read(&path).await {
        Ok(bytes) => {
            ...
            app_log_info!("✅ READ BINARY: Successfully read {} bytes from {}", bytes.len(), path);
            Ok(base64_content)
        }
        Err(e) => {
...
#[tauri::command]
pub fn list_directory_recursive(path: String, state: State<'_, AppState>) -> Result<Vec<FileItem>, String> {
    state.file_service.list_directory_recursive(&path)
        .map_err(|e| format!("Failed to list directory recursively: {}", e))
}

/// Get all indexed files
#[tauri::command]
pub async fn get_indexed_files(state: State<'_, AppState>) -> Result<Vec<ImageVectorDataResponse>, String> {
...
fn group_files_by_file_path(files: Vec<ImageVectorDataResponse>) -> Vec<ImageVectorDataResponse> {
    ...
    let mut video_frames: std::collections::HashMap<String, Vec<ImageVectorDataResponse>> = std::collections::HashMap::new();
    for file in files {
        ...
        if is_video_frame {
            ...
            video_frames.entry(video_path).or_insert_with(Vec::new).push(file);
        } else {
...
#[tauri::command]
pub async fn get_indexed_files_grouped(state: State<'_, AppState>) -> Result<Vec<ImageVectorDataResponse>, String> {    
    match state.sqlite_service.get_all_images() {
        Ok(results) => {
            Ok(group_files_by_file_path(results))
        },
        Err(_) => {
            Ok(Vec::new())
        }
...
#[tauri::command]
pub async fn get_indexed_files_grouped_paginated(offset: usize, limit: usize, state: State<'_, AppState>) -> Result<Vec<ImageVectorDataResponse>, String> {
    match state.sqlite_service.get_all_images() {
        Ok(results) => {
            // First group the files
            let grouped_results = group_files_by_file_path(results);
            ...
            let paginated_results = grouped_results.into_iter().skip(start).take(end - start).collect();
...
pub async fn get_indexed_count(state: State<'_, AppState>) -> Result<usize, String> {
    match state.sqlite_service.get_all_images() {
        Ok(results) => Ok(count_unique_files_by_file_path(results)),
        Err(_) => Ok(0)
    }
}
...
#[tauri::command]
pub async fn set_indexed_directory(state: State<'_, AppState>, is_set_default: bool,  new_dir: String) -> Result<String, String>  {
    app_log_info!("🔧 SET_INDEX_DIR: Starting set_indexed_directory command");
    app_log_info!("🔧 SET_INDEX_DIR: is_set_default = {}, new_dir = '{}'", is_set_default, new_dir);
    ...
    if is_set_default {
        ...
        match state.sqlite_service.set_db_path(None) {
            Ok(file_path) => {
                app_log_info!("✅ SET_INDEX_DIR: Successfully set to default: {}", file_path);
                Ok(file_path)
            }
            Err(e) => {
        ...
        match state.file_service.list_directory(&new_dir) {
            Ok(_) => {
                app_log_info!("✅ SET_INDEX_DIR: Directory access confirmed for: {}", new_dir);
                
                match state.sqlite_service.set_db_path(Some(&new_dir)) {
                    Ok(file_path) => {
                        app_log_info!("✅ SET_INDEX_DIR: Successfully set database path: {}", file_path);
                        Ok(file_path)
                    }
                    Err(e) => {
            ...
            Err(e) => {
                app_log_error!("❌ SET_INDEX_DIR: Directory access failed for '{}': {}", new_dir, e);
...
pub async fn clean_stale_entries(state: State<'_, AppState>) -> Result<String, String> {
    ...
    app_log_info!("✅ CLEANUP: Removed {} stale entries out of {} total files", removed_count, total_files);
