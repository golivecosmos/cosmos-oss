...
async fn validate_google_api_key(api_key: &str) -> Result<(), String> {
    if !looks_like_google_api_key(api_key) {
        return Err("API key format looks invalid. Google API keys usually start with 'AIza'.".to_string());
    }
    ...
    .await
    .map_err(|_| "Timed out while validating Google API key. Check your network and try again.".to_string())?
    ...
    Err(categorize_google_api_error(status, message, "Google API key validation"))
}
...
pub async fn uninstall_app(
    ...
    match result {
        Ok(response) => {
            println!("🔍 UNINSTALL COMMAND: Successfully uninstalled app, response: {:?}", response);
            Ok(response)
        }
        Err(e) => {
