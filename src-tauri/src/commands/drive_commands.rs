...
#[tauri::command]
pub async fn get_connected_drives(state: State<'_, AppState>) -> Result<Vec<DriveInfo>, String> {
        ...
        Ok(drives) => {
            ...
            for drive in &drives {
                if let Err(e) = state.sqlite_service.update_drive_status(
                    ...
                    app_log_warn!(
                        "⚠️ DRIVE: Failed to update status for {}: {}",
                        drive.name,
                        e
                    );
                }
...
pub async fn get_drive_info(
    ...
    match state.drive_service.get_drive_info(&drive_uuid).await {
        Some(drive_info) => {
            app_log_info!(
                "✅ DRIVE COMMAND: Found drive info for: {}",
                drive_info.name
            );
            Ok(Some(drive_info))
        }
...
#[tauri::command]
pub async fn refresh_drives(state: State<'_, AppState>) -> Result<Vec<DriveInfo>, String> {
...
#[tauri::command]
pub async fn get_all_drives(state: State<'_, AppState>) -> Result<Vec<DriveInfo>, String> {
...
pub async fn update_drive_status(
    ...
    app_log_info!(
        "🔄 DRIVE COMMAND: Updating drive {} status to: {}",
        drive_uuid,
        status
    );
    ...
    match state
        .drive_service
        .update_drive_status(&drive_uuid, drive_status)
        .await
    {
        Ok(_) => {
...
pub async fn is_path_on_external_drive(
    ...
    app_log_debug!(
        "🔍 DRIVE COMMAND: Checking if path is on external drive: {}",
        path
    );
    ...
    match state.drive_service.get_drive_for_path(&path).await {
        Some(drive_info) => {
            ...
            app_log_debug!(
                "✅ DRIVE COMMAND: Path is on {} drive: {}",
...
pub async fn get_drive_indexed_files(
    ...
    app_log_info!(
        "🔍 DRIVE COMMAND: Getting indexed files for drive: {}",
        drive_uuid
    );
...
pub async fn get_drive_stats(
    ...
    match state.drive_service.get_drive_info(&drive_uuid).await {
        ...
        None => {
            app_log_warn!(
                "⚠️ DRIVE COMMAND: Drive not found for stats: {}",
                drive_uuid
            );
...
pub async fn update_drive_metadata(
    ...
    app_log_info!(
        "🔄 DRIVE COMMAND: Updating metadata for drive: {} (name: {:?}, location: {:?})",
        drive_uuid,
        custom_name,
        physical_location
    );
...
pub async fn delete_drive_from_database(
    ...
    app_log_info!(
        "🗑️ DRIVE COMMAND: Deleting drive from database: {}",
        drive_uuid
    );
    ...
    match state.sqlite_service.delete_drive(&drive_uuid) {
        Ok(_) => {
            app_log_info!(
                "✅ DRIVE COMMAND: Deleted drive successfully: {}",
                drive_uuid
            );
            Ok(())
        }
...
pub async fn get_all_drives_with_metadata(
    ...
    match state.sqlite_service.get_all_drives() {
        Ok(drives) => {
            app_log_info!(
                "✅ DRIVE COMMAND: Found {} drives with metadata",
                drives.len()
            );
            Ok(drives)
        }
        Err(e) => {
            app_log_error!(
                "❌ DRIVE COMMAND: Failed to get drives with metadata: {}",
                e
            );
...
pub async fn get_drive_metadata(
    ...
    app_log_info!(
        "🔍 DRIVE COMMAND: Getting metadata for drive: {}",
        drive_uuid
    );
    ...
    match state.sqlite_service.get_drive_by_uuid(&drive_uuid) {
        Ok(drive) => {
            if drive.is_some() {
                ...
                app_log_warn!(
                    "⚠️ DRIVE COMMAND: No metadata found for drive: {}",
                    drive_uuid
                );
            }
            Ok(drive)
...
#[tauri::command]
pub async fn sync_drives_to_database(state: State<'_, AppState>) -> Result<(), String> {
    ...
    match state.drive_service.detect_connected_drives().await {
        Ok(connected_drives) => {
                ...
                match state.sqlite_service.get_drive_by_uuid(&drive.uuid) {
                    Ok(Some(_)) => {
                        ...
                        if let Err(e) = state.sqlite_service.update_drive_status(
                            ...
                            app_log_warn!(
                                "⚠️ DRIVE SYNC: Failed to update status for {}: {}",
                                drive.name,
                                e
                            );
                        }
                    ...
                    Ok(None) => {
                        ...
                        if let Err(e) = state.sqlite_service.add_drive(
                            ...
                            app_log_warn!(
                                "⚠️ DRIVE SYNC: Failed to add new drive {}: {}",
                                drive.name,
                                e
                            );
                        } else {
                            app_log_info!(
                                "✅ DRIVE SYNC: Added new drive {} to database",
                                drive.name
                            );
                        }
                    }
                    Err(e) => {
                        app_log_warn!(
                            "⚠️ DRIVE SYNC: Failed to check if drive {} exists: {}",
                            drive.name,
                            e
                        );
                    }
                ...
                if let Some(uuid) = db_drive.get("uuid").and_then(|v| v.as_str()) {
                    if !connected_uuids.contains(uuid) {
                        if let Err(e) =
                            state
                                .sqlite_service
                                .update_drive_status(uuid, "disconnected", None)
                        {
                            app_log_warn!(
                                "⚠️ DRIVE SYNC: Failed to mark drive {} as disconnected: {}",
                                uuid,
                                e
                            );
                        }
            ...
            app_log_info!(
                "✅ DRIVE SYNC: Synchronized {} connected drives",
                connected_drives.len()
            );
            Ok(())
        }
        Err(e) => {
