...
pub async fn check_models_status(_state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    ...
    app_log_info!("📊 MODEL STATUS: Available = {}, Missing = {}", models_available, missing_models.len());
...
#[tauri::command]
pub async fn clear_and_redownload_models(app_handle: tauri::AppHandle, state: State<'_, AppState>) -> Result<String, String> {
    ...
    // Download all models
    match download_service.download_all_missing_models(progress_callback).await {
        Ok(_) => {
            ...
            match state.model_service.reload_clip_model().await {
                ...
                Err(e) => {
                    app_log_error!("❌ AUTO RELOAD: Failed to reload models after re-download: {}", e);
                    // Still return success since download worked, just mention reload issue
                    Ok(format!("Models downloaded successfully, but failed to reload: {}", e))
                }
            }
        },
        Err(e) => {
...
#[tauri::command]
pub async fn download_models(app_handle: tauri::AppHandle, state: State<'_, AppState>) -> Result<String, String> {
    ...
    match download_result {
        ...
        Err(_) => {
            ...
            Err("Download timed out. Please check your internet connection and try again.".to_string())
        }
    }
}
...
pub async fn debug_model_status(state: State<'_, AppState>) -> Result<String, String> {
    ...
    let models_available = DownloadService::are_models_available();
    app_log_info!("🔧 Download service says models available: {}", models_available);
    ...
    match DownloadService::check_missing_models() {
        Ok(missing) => {
            ...
            for model in &missing {
                app_log_info!("🔧 Missing: {} at {}", model.name, model.destination_path.display());
            }
        },
        Err(e) => {
            app_log_error!("🔧 Failed to check missing models: {}", e);
    ...
    match crate::utils::path_utils::get_app_data_dir() {
        Ok(app_dir) => {
            ...
            app_log_info!("🔧 Text model directory: {} - {}", text_model_dir.display(), if text_model_dir.exists() { "✅" } else { "❌" });
            app_log_info!("🔧 Vision model directory: {} - {}", vision_model_dir.display(), if vision_model_dir.exists() { "✅" } else { "❌" });
            ...
            app_log_info!("🔧 Old onnx directory: {} - {}", old_onnx_dir.display(), if old_onnx_dir.exists() { "✅" } else { "❌" });
            ...
            if old_onnx_dir.exists() {
                ...
                app_log_info!("🔧 Old text model: {} - {}", old_text_model.display(), if old_text_model.exists() { "✅" } else { "❌" });
                app_log_info!("🔧 Old vision model: {} - {}", old_vision_model.display(), if old_vision_model.exists() { "✅" } else { "❌" });
            }
        },
        Err(e) => {
    ...
    Ok(format!("Debug complete - check logs for details. Model loaded: {}, Files available: {}", is_loaded, models_available))
}
